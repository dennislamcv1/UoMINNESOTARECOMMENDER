...
public class SVDModelBuilder implements Provider<SVDModel> {
    ...
    public SVDModel get() {
        ...
        if (featureCount > 0) {
            ...
            int nfeatures = Math.min(featureCount, weights.getDimension());
            userMatrix = userMatrix.getSubMatrix(0, userMatrix.getRowDimension() - 1,
                                                 0, nfeatures - 1);
            itemMatrix = itemMatrix.getSubMatrix(0, itemMatrix.getRowDimension() - 1,
                                                 0, nfeatures - 1);
            weights = weights.getSubVector(0, nfeatures);
        }
    ...
    private RealMatrix createRatingMatrix(KeyIndex userIndex, KeyIndex itemIndex) {
        ...
        try (ObjectStream<Rating> ratings = dao.query(Rating.class)
            ...
            for (Rating rating : ratings) {
                ...
                int uidx = userIndex.getIndex(user);
                int iidx = itemIndex.getIndex(item);

                double bias = baseline.getIntercept()
                              + baseline.getUserBias(user)
                              + baseline.getItemBias(item);
                double residual = rating.getValue() - bias;
                matrix.setEntry(uidx, iidx, residual);
            }
        }
