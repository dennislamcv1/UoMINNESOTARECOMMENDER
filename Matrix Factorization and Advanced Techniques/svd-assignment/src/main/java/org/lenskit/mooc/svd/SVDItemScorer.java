...
public class SVDItemScorer extends AbstractItemScorer {
    ...
    public ResultMap scoreWithDetails(long user, @Nonnull Collection<Long> items) {
        ...
        double userBias = baseline.getUserBias(user);
        double intercept = baseline.getIntercept();
        RealVector weightedUserFeatures = userFeatures.ebeMultiply(model.getFeatureWeights());
        ...
        for (long item : itemSet) {
            ...
            double offset = weightedUserFeatures.dotProduct(itemFeatures);
            double score = intercept + userBias + baseline.getItemBias(item) + offset;
